...
const images = [
  { id: 1, src: image1, alt: 'Image 1', span: 'col-span-2 row-span-1' },
  { id: 2, src: image2, alt: 'Image 2', span: 'col-span-2 row-span-1' },
  ...
  return (
    <div className="flex justify-center w-full h-[100vh] items-center min-h-screen p-4">
      <div className="w-full max-w-6xl h-full rounded-xl shadow-lg p-4">
        <div className="grid grid-cols-4  h-full grid-rows-3 gap-3 auto-rows-[minmax(100px,_auto)]">
          {images.map((image) => (
            <div 
            key={image.id} 
            className={`${image.span} border-[3px] relative rounded-lg overflow-hidden ${image.id === 1 || image.id === 3 ? 'border-4' : ''}`}
            style={{
